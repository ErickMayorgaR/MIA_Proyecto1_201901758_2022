...
void RmUsr::executeRmUsr() {
    if (name == "")
        return coutError("Error: faltan parámetros obligatorios.", NULL);

    if (!_user_logged.logged_in || _user_logged.nombre != "root")
        ...
        switch (count) {
            case 4:
                ...
                if (user_tmp.nombre == user_to_remove.nombre && user_tmp.UID != 0) {
                    tmp = std::to_string(user_tmp.UID) + ",U," + user_tmp.grupo + "," + user_tmp.nombre + "," +
                          user_tmp.contrasena + "\n";
                }
                break;
            default:
    ...
    if (tmp == "")
        return coutError("Error: No existe ningún usuario activo con el nombre: '" + name + "'.", file);
