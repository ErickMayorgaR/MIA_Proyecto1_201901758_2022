...
Block::Block(ParticionesMontadas mounted) {
    this->mounted =  mounted;
}

std::string Block::getReport() {
    ...
    grafo += getDotBlock(0, super_bloque.s_inode_start, super_bloque.s_block_start,
                         mounted.path);
    grafo += "}";
...
std::string Block::getDotBlock(int _index_inode, int _start_inodes, int _start_blocks, std::string _path) {
...
std::string Block::getDot_folder_block(int _start_blocks, int _index_block, std::string _path) {
    FILE *_file = fopen(_path.c_str(), "rb");
    CarpetasBlock cb;
    fseek(_file, _start_blocks, SEEK_SET);
...
std::string Block::getDot_file_block(int _start_blocks, int _index_block, std::string _path) {
    FILE *_file = fopen(_path.c_str(), "rb");
    ArchivosBlock ab;
    fseek(_file, _start_blocks, SEEK_SET);
    fseek(_file, _index_block * 64, SEEK_CUR);
...
std::string Block::getDot_pointer_block(int _start_blocks, int _index_block, std::string _path) {
    FILE *_file = fopen(_path.c_str(), "rb");
    ApuntadoresBlock ab;
    fseek(_file, _start_blocks, SEEK_SET);
    fseek(_file, _index_block * 64, SEEK_CUR);
