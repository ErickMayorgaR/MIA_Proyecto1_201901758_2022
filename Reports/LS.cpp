...
LS::LS(ParticionesMontadas mounted) {
    this->mounted = mounted;
}


std::string LS::getReport() {
    ...
    grafo += std::string("\"Ls Report\" [margin=\"1\" label = <\n") +
...
std::string LS::getDotLS(FILE *_file, int _index_inode, int _start_inodes, int _start_blocks, std::string _dot,
                         std::string _name) {
    ...
    /* Agregar al dot */
    std::string i_ctime = ctime(&inode_current.i_ctime);
    std::string i_mtime = ctime(&inode_current.i_mtime);
    _dot += std::string("<TR>\n") +
    ...
    if (inode_current.i_type == '0') {
        ...
        // falta indirectos
        for (int i = 0; i < 15; i++) {
            if (inode_current.i_block[i] !=-1) { /* Leer el bloque y redireccionar al inodo y ver si de nuevo es otra carpeta */
