...
void ReportHandler::executeReportHandler() {
    ...
    if (_name == "mbr") {
        ...
        if (_name == "inode") {
            ...
            FileReport *fileReport = new FileReport(mounted);
            grafo = fileReport->getReport(_ruta);
        }
        else if (_name == "ls") {
            LS *ls = new LS(mounted);
            grafo = ls->getReport();
        }
        else if (_name == "journaling") {
            JournalingReport *journaling = new JournalingReport();
            journaling->generateReport(dir_output);
        }
        else if (_name == "tree") {
            Tree *tree = new Tree(mounted);
            grafo = tree->getReport();
        }
        else
