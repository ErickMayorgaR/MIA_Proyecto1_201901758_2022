...
void Remove::executeRemove() {
    ...
    return deleteFile(npath, name);
}


void Remove::deleteFile(std::string _path, std::string _name) {
    ...
    for (int i = 0; i < 15 && !x; i++) // Obtener el inodo asociado directo
    {
        if (inode_father.i_block[i] != -1) {
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (std::string(file_block.b_content[k].b_name) == _name) {
    ...
    toRemove(_index_inode, start_byte_sb);
}


int Remove::toRemove(int _index_inode, int _seek_superbloque) {
...
int Remove::rewriteInode(int _index_inode, int _seek_superbloque) {
    FILE *_file = fopen((_user_logged.mounted.path).c_str(), "rb+");
    /* Lectura del superbloque */
    Superbloque super_bloque;
    fseek(_file, _seek_superbloque, SEEK_SET);
    ...
    if (!HasPermission(_user_logged, inode_current, 2)) {
        ...
        coutError("El usuario no posee el permiso de escritura de todos los archivos o subcarpetas.", _file);
        return 0;
    } else {
